...
function Card({ roomdata, fromdate, todate }) {
  return (
    <>
      <div className="card my-2" style={{ width: "25rem" }}>
        ...
        <div className="card-body">
          ...
          <h5 className="mb-4 fst-italic">
            ₹ {roomdata.rentperday}
            <span className="fw-light text-decoration-line-through ms-2">
              ₹ {roomdata.rentperday * 2}
            </span>
            <span className="text-success"> 50% off</span>
          </h5>
          {fromdate && todate && (
            <Link to={`booking/${roomdata._id}/${fromdate}/${todate}`}>
              <button className="btn btn-sm btn-warning text-white w-50 d-block ms-auto">
                Book Now
              </button>
            </Link>
          )}
